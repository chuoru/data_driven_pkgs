...
class DeePC(object):
    ...
    def __init__(self, data: Data, Tini: int, horizon: int):
        ...
        @param explained_variance: Regularization term in (0,1] used to 
        approximate the Hankel matrices.
    ...
    def update_data(self, data: Data):
        """! Update Hankel matrices of DeePC. You need to rebuild the
        optimization problem after calling this funciton.
        @param data: A tuple of input/output data. Data should have shape TxM,
        where T is the batch size and M is the number of features
        """
        assert len(data.u.shape) == 2, \
            "Data needs to be shaped as a TxM matrix (T is the number of"\
        ...
        assert data.y.shape[0] - self.Tini - self.horizon + 1 >= 1, \
            "There is not enough data: this value"\
            f"{data.y.shape[0] - self.Tini - self.horizon + 1}"
        "needs to be >= 1"

        self.Up, self.Uf, self.Yp, self.Yf = split_data(
            data, self.Tini, self.horizon)
    ...
    def build_problem(self,
                      build_loss: Callable[[cp.Variable, cp.Variable],
                                           Expression],
                      build_constraints: Optional[Callable[[
                          cp.Variable, cp.Variable], Optional[List[Constraint]
                                                              ]]] = None,
                      lambda_g: float = 0.,
                      lambda_y: float = 0.,
                      lambda_u: float = 0.,
        ...
        @param build_loss: Callback function that takes as input an
        (input,output) tuple of data
        @param build_constraints: Callback function that takes as input an
        (input,output) tuple of data
        @param lambda_g: non-negative scalar. Regularization factor for g.
        Used for stochastic/non-linear systems.
        @param lambda_y: non-negative scalar. Regularization factor for y_init.
        Used for stochastic/non-linear systems.
        @param lambda_u: non-negative scalar. Regularization factor for u_init.
        Used for stochastic/non-linear systems.
        @param lambda_proj: Positive term that penalizes the least square
        solution.
        @return Parameters of the optimization problem
        """
        assert build_loss is not None, "Loss function callback cannot be none"

        assert lambda_g >= 0 and lambda_y >= 0, "Regularizers must be non-"\
            "negative"

        assert lambda_u >= 0, "Regularizer of u_init must be non-negative"

        assert lambda_proj >= 0, "The projection regularizer must be non-"\
            "negative"
        ...
        g = cp.Variable(shape=(self.T - self.Tini -
                        self.horizon + 1), name='g')
        ...
        if math.isclose(lambda_y, 0):
            constraints.append(cp.norm(slack_y, 2) <= DeePC._eps)
        if math.isclose(lambda_u, 0):
            constraints.append(cp.norm(slack_u, 2) <= DeePC._eps)

        # u, y = self.Uf @ g, self.Yf @ g
        u = cp.reshape(u, (self.horizon, self.M))
        y = cp.reshape(y, (self.horizon, self.P))

        _constraints = build_constraints(
            u, y) if build_constraints is not None else (None, None)

        for idx, constraint in enumerate(_constraints):
            if constraint is None or not isinstance(
                    constraint, Constraint) or not constraint.is_dcp():
                raise Exception(
                    f'Constraint {idx} is not defined or is not convex.')
        ...
        if _loss is None or not isinstance(
                _loss, Expression) or not _loss.is_dcp():
            raise Exception('Loss function is not defined or is not convex!')

        # Add regularizers
        _regularizers = lambda_g * \
            cp.norm(g, p=1) if lambda_g > DeePC._eps else 0
        _regularizers += lambda_y * \
            cp.norm(slack_y, p=1) if lambda_y > DeePC._eps else 0
        _regularizers += lambda_proj * \
            cp.norm(I_min_P @ g) if lambda_proj > DeePC._eps else 0
        _regularizers += lambda_u * \
            cp.norm(slack_u, p=1) if lambda_u > DeePC._eps else 0
        ...
        try:
            ...
            raise Exception(
                f'Error while constructing the DeePC problem. Details: {e}')

        self.optimization_problem = OptimizationProblem(
            variables=OptimizationProblemVariables(
                u_ini=uini, y_ini=yini, u=u, y=y, g=g, slack_y=slack_y,
                slack_u=slack_u),
    ...
    def solve(
        ...
        **cvxpy_kwargs
        ...
        """
        assert len(
            data_ini.u.shape) == 2, "Data needs to be shaped as a TxM matrix (T is the number of samples and M is the number of features)"
        assert len(
            data_ini.y.shape) == 2, "Data needs to be shaped as a TxM matrix (T is the number of samples and M is the number of features)"
        ...
        uini, yini = data_ini.u[:self.Tini].flatten(
        ), data_ini.y[:self.Tini].flatten()
        ...
        try:
            ...
            raise Exception(
                f'Error while solving the DeePC problem. Details: {e}')

        if np.isinf(result):
            raise Exception('Problem is unbounded')

        u_optimal = (
            self.Uf @ self.optimization_problem.variables.g.value).reshape(self.horizon, self.M)
        info = {
            'value': result,
