...
EXPERIMENT_HORIZON = 1 # Total number of steps
...
A = np.array([
        ...
        [0.24664, 0.70469, 0],
        [0., 0., 1.]])
B = np.array([[0.75937], [0.12515], [0.]])
C = np.array([[0., 0., 1.]])
D = np.zeros((C.shape[0], B.shape[1]))

sys = System(scipysig.StateSpace(A, B, C, D, dt=1), x0 = np.array([0., 0., 0.]))
...
for T in T_list:
    ...
    data = sys.apply_input(u = np.random.normal(size=[nu, T]).reshape((T, nu)), noise_std=0, model=DifferentialDrive(wheel_base=0.5))
    deepc = DeePC(data, Tini = T_INI, horizon = HORIZON)

    # Create initial data
    data_ini = Data(u = np.zeros((T_INI, nu)), y = np.zeros((T_INI, nx)))
    ...
    for _ in range(EXPERIMENT_HORIZON//s):
        ...
        _ = sys.apply_input(u = u_optimal[:s, :], noise_std=1e-2, model=DifferentialDrive(wheel_base=0.5))
